...
// Uploads a single team-member portrait to Vercel Blob and returns its URL.
// The URL is then attached to a member and persisted via POST /api/admin/team.
export async function POST(request: Request) {
  ...
  try {
    const url = await uploadImage(`team-${Date.now()}`, file);
    return NextResponse.json({ ok: true, url });
  } catch (err) {
    console.error("[admin/team/photo] upload error:", err);
